...
class TemplateUtils:
    ...
    def render_template_from_file(
        file_path: str, variables: Dict[str, Any]
    ) -> Optional[str]:
        """
        从文件渲染模板
    ...
    @staticmethod
    def render_jinja2_template_from_file(
        file_path: str, variables: Dict[str, Any]
    ) -> Optional[str]:
        """
        使用Jinja2从文件渲染模板
    ...
    def render_template_with_filters(
        template: str, variables: Dict[str, Any], filters: Dict[str, Callable]
    ) -> str:
        """
        使用自定义过滤器渲染模板
    ...
    @staticmethod
    def render_template_with_loops(template: str, variables: Dict[str, Any]) -> str:
        """
        渲染带循环的模板
        ...
        pattern = (
            r"\{%\s*for\s+([^\s]+)\s+in\s+([^%]+)\s*%\}([\s\S]*?)\{%\s*endfor\s*%\}"
        )
    ...
    def save_rendered_template(
        template: str, variables: Dict[str, Any], output_path: str
    ) -> bool:
        """
        渲染模板并保存到文件
    ...
    def save_rendered_template_from_file(
        file_path: str, variables: Dict[str, Any], output_path: str
    ) -> bool:
        """
        从文件渲染模板并保存到文件
