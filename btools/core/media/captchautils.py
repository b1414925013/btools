...
class CaptchaUtils:
    ...
    def generate_captcha(
        length: int = 4,
        captcha_type: int = TYPE_ALPHANUMERIC,
        width: int = 120,
        height: int = 40,
        font_size: int = 24,
        noise_level: int = 2,
    ...
    def generate_math_captcha(
        width: int = 120, height: int = 40, font_size: int = 24, noise_level: int = 2
    ) -> Tuple[str, str, bytes]:
        """
        生成数学计算验证码
    ...
    @staticmethod
    def _add_noise(
        draw: ImageDraw.ImageDraw, width: int, height: int, noise_level: int
    ):
        """
        添加噪点
    ...
    def validate_captcha(
        user_input: str, correct_captcha: str, case_sensitive: bool = False
    ) -> bool:
        """
        验证验证码
    ...
    def generate_complex_captcha() -> Tuple[str, bytes]:
        ...
        """
        return CaptchaUtils.generate_captcha(
            length=6, width=160, height=50, noise_level=4
        )
