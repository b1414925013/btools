...
class CompressUtils:
    ...
    @staticmethod
    def zip_directory(
        directory: str, output_path: str, include_root: bool = False
    ) -> None:
        """
        压缩目录为ZIP文件
    ...
    @staticmethod
    def compress_file(
        file_path: str, output_path: str, compression_type: str = "zip"
    ) -> None:
        """
        压缩文件（根据指定类型）
    ...
    @staticmethod
    def decompress_file(
        compressed_path: str, output_path: Optional[str] = None
    ) -> None:
        """
        解压文件（自动检测类型）
