...
from docx.enum.text import WD_BREAK_TYPE, WD_PARAGRAPH_ALIGNMENT
...
class WordUtils:
    ...
    @staticmethod
    def add_paragraph(
        document: Document, text: str, style: Optional[str] = None
    ) -> Any:
        """
        添加段落
    ...
    @staticmethod
    def add_table(
        document: Document, rows: int, cols: int, style: Optional[str] = None
    ) -> Any:
        """
        添加表格
    ...
    @staticmethod
    def add_picture(
        document: Document, image_path: str, width: Optional[float] = None
    ) -> Any:
        """
        添加图片
    ...
    def read_document(file_path: str) -> Dict[str, Any]:
        ...
        content = {"headings": [], "paragraphs": [], "tables": []}

        # 读取标题和段落
        for para in document.paragraphs:
            if para.style.name.startswith("Heading"):
                content["headings"].append(
                    {"text": para.text, "level": int(para.style.name.split(" ")[-1])}
                )
            else:
    ...
    def create_simple_report(
        file_path: str, title: str, content: Dict[str, Any]
    ) -> None:
        """
        创建简单的Word报告
