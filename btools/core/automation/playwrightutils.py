...
class PlaywrightUtils:
    ...
    @staticmethod
    def find_locator(
        page: Page, locator: str, timeout: Optional[int] = None
    ) -> Locator:
        """
        查找元素
    ...
    def get_element_attribute(
        page: Page, locator: str, attribute: str, timeout: Optional[int] = None
    ) -> str:
        """
        获取元素属性
    ...
    def switch_to_window(page: Page, index: int = 1) -> Page:
        ...
        if contexts:
            ...
            if index < len(pages):
                ...
                raise IndexError(
                    f"窗口索引 {index} 超出范围，当前只有 {len(pages)} 个窗口"
                )
        else:
    ...
    def wait_for_load_state(
        page: Page, state: str = "networkidle", timeout: Optional[int] = None
    ) -> None:
        """
        等待页面加载状态
    ...
    def drag_and_drop(
        ...
        page.locator(source_locator).drag_to(
            page.locator(target_locator), timeout=timeout
        )

    @staticmethod
    def select_dropdown_option(
        page: Page, locator: str, option: Union[str, int], timeout: Optional[int] = None
    ) -> None:
        """
        选择下拉框选项
    ...
    @staticmethod
    def fill_form(
        page: Page, form_data: Dict[str, str], timeout: Optional[int] = None
    ) -> None:
        """
        填充表单
...
class PlaywrightElement:
    ...
    def send_keys(
        self, text: str, clear_first: bool = True, timeout: Optional[int] = None
    ) -> None:
        """
        输入文本
    ...
    def get_attribute(self, attribute: str, timeout: Optional[int] = None) -> str:
        ...
        Returns:
            str: 元素属性值
        """
        return PlaywrightUtils.get_element_attribute(
            self.page, self.locator, attribute, timeout
        )
