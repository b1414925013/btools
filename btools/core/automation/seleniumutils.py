...
class SeleniumUtils:
    ...
    def click_element(
        ...
        Args:
            ...
            poll_frequency (float): 轮询频率（秒）
        """
        element = SeleniumUtils.wait_for_element_visible(
    ...
    def is_element_visible(
        ...
        """
        try:
            SeleniumUtils.wait_for_element_visible(
                driver, by, value, timeout, poll_frequency
            )
            return True
        except Exception:
    ...
    def switch_to_window(driver: webdriver, window_index: int = 1) -> None:
        ...
        if window_index < len(windows):
            ...
            raise IndexError(
                f"窗口索引 {window_index} 超出范围，当前只有 {len(windows)} 个窗口"
            )
    ...
    def drag_and_drop(
        ...
        Args:
            ...
            poll_frequency (float): 轮询频率（秒）
        """
        source_element = SeleniumUtils.wait_for_element_visible(
            driver, source_by, source_value, timeout, poll_frequency
        )
        target_element = SeleniumUtils.wait_for_element_visible(
            driver, target_by, target_value, timeout, poll_frequency
        )
        actions = ActionChains(driver)
...
class SeleniumElement:
    ...
    def send_keys(self, text: str, clear_first: bool = True) -> None:
        ...
        Args:
            ...
            clear_first (bool): 是否先清空输入框
        """
        SeleniumUtils.send_keys(
            self.driver, self.by, self.value, text, clear_first=clear_first
        )
    ...
    def get_attribute(self, attribute: str) -> str:
        ...
        Returns:
            str: 元素属性值
        """
        return SeleniumUtils.get_element_attribute(
            self.driver, self.by, self.value, attribute
        )
