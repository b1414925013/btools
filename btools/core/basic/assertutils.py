...
class AssertUtil:
    ...
    def is_instance(obj: Any, cls: type, message: Optional[str] = None) -> None:
        ...
        if not isinstance(obj, cls):
            raise AssertionError(
                message or f"对象不是 {cls.__name__} 类型: {type(obj)}"
            )
    ...
    def matches(pattern: str, string: str, message: Optional[str] = None) -> None:
        ...
        if not re.match(pattern, string):
            raise AssertionError(
                message or f"字符串 '{string}' 不匹配正则表达式 '{pattern}'"
            )
    ...
    def not_matches(pattern: str, string: str, message: Optional[str] = None) -> None:
        ...
        if re.match(pattern, string):
            raise AssertionError(
                message or f"字符串 '{string}' 匹配正则表达式 '{pattern}'"
            )
    ...
    def raises(expected_exception: type, func: Callable, *args, **kwargs) -> None:
        ...
        try:
            ...
            if not isinstance(e, expected_exception):
                raise AssertionError(
                    f"抛出了异常 {type(e).__name__}，但期望的是 {expected_exception.__name__}"
                )
    ...
    @staticmethod
    def greater(
        a: Union[int, float], b: Union[int, float], message: Optional[str] = None
    ) -> None:
        """
        断言 a 大于 b
    ...
    def between(
        ...
        if not (min_val <= value <= max_val):
            raise AssertionError(
                message or f"{value} 不在 [{min_val}, {max_val}] 范围内"
            )
    ...
    def has_length(collection: Any, length: int, message: Optional[str] = None) -> None:
        ...
        if len(collection) != length:
            raise AssertionError(
                message or f"集合长度为 {len(collection)}，期望为 {length}"
            )
    ...
    def assert_json_equals(
        actual: Union[Dict, str], expected: Union[Dict, str], message: str = None
    ):
        """
        断言JSON相等
    ...
    def assert_response_status(response, expected_status: int, message: str = None):
        ...
        if actual_status != expected_status:
            msg = (
                message or f"状态码不匹配: 实际={actual_status}, 期望={expected_status}"
            )
            # 尝试获取响应内容作为错误信息的一部分
            try:
