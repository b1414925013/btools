...
class ExceptionUtils:
    ...
    def format_exception(exception: Exception, include_stack: bool = True) -> str:
        ...
        parts.append(
            f"Exception Message: {ExceptionUtils.get_exception_message(exception)}"
        )
        parts.append(
            f"Exception Module: {ExceptionUtils.get_exception_module(exception)}"
        )

        cause = ExceptionUtils.get_cause(exception)
        if cause:
            parts.append(
                f"Cause: {ExceptionUtils.get_exception_type(cause)}: {ExceptionUtils.get_exception_message(cause)}"
            )
        ...
        if suppressed:
            ...
            for i, sup in enumerate(suppressed):
                parts.append(
                    f"  {i+1}. {ExceptionUtils.get_exception_type(sup)}: {ExceptionUtils.get_exception_message(sup)}"
                )
    ...
    def get_exception_chain_strings(exception: Exception) -> List[str]:
        ...
        return [
            f"{ExceptionUtils.get_exception_type(ex)}: {ExceptionUtils.get_exception_message(ex)}"
            for ex in chain
        ]
    ...
    @staticmethod
    def catch_and_log(
        func: callable, logger: Optional[Any] = None, *args, **kwargs
    ) -> Optional[Any]:
        """
        捕获并记录异常
    ...
    def serialize_exception(exception: Exception) -> Dict[str, Any]:
        ...
        return {
            ...
            "stack_trace": ExceptionUtils.get_stack_trace(exception),
            "cause": (
                ExceptionUtils.serialize_exception(ExceptionUtils.get_cause(exception))
                if ExceptionUtils.get_cause(exception)
                else None
            ),
            "suppressed": [
                ExceptionUtils.serialize_exception(sup)
                for sup in ExceptionUtils.get_suppressed(exception)
            ],
        }
