...
`decorator` 方法用于创建一个简单的基础装饰器，它会保留原始函数的元数据并直接调用原始函数，不添加任何额外功能。
...
# 使用 decorator 创建装饰器
greet_decorator = DecoratorUtil.decorator(greet)
...
# 使用 decorator 创建装饰器
bold_decorator = DecoratorUtil.decorator(make_bold)
...
# 创建重试装饰器
retry_decorator = DecoratorUtil.retry(max_retries=3, delay=0.5)
...
# 创建过时警告装饰器
deprecation_decorator = DecoratorUtil.deprecation("此方法已过时，请使用 new_function() 代替")
...
# 创建异步计时装饰器
async_timer_decorator = DecoratorUtil.createAsyncTimerDecorator()
...
# 运行事件循环
asyncio.run(main())
# 输出: async_slow_function 执行耗时: 1.0000 秒
```
...
# 创建带有过期时间的高级缓存装饰器（2秒过期）
advanced_cache_decorator = DecoratorUtil.advanced_cache(ttl=2)
...
# 创建上下文管理装饰器
db_context_decorator = DecoratorUtil.context_manager(DatabaseConnection())
...
result = api_request()
print(result)
# 输出: [指标] api_request: 耗时=0.3000s, 成功=True, 标签={'endpoint': '/users', 'method': 'GET'}
```
...
# 组合装饰器
combined_decorator = DecoratorUtil.combine(timer_decorator, logging_decorator, cache_decorator)
...
# 检查是否为装饰器
print(f"decorated_function 是装饰器: {DecoratorUtil.is_decorator(decorated_function)}")
print(f"regular_function 是装饰器: {DecoratorUtil.is_decorator(regular_function)}")

# 获取原始函数
original_function = DecoratorUtil.get_original_function(decorated_function)
...
# 获取装饰器链
chain = DecoratorUtil.get_decorator_chain(multi_decorated_function)
...
# 创建条件装饰器
conditional_cache_decorator = DecoratorUtil.conditional(should_cache, cache_decorator)
...
# 检查装饰器
print(f"\ncomplex_operation 是装饰器: {DecoratorUtil.is_decorator(complex_operation)}")

# 获取原始函数
original = DecoratorUtil.get_original_function(complex_operation)
print(f"原始函数名: {original.__name__}")

# 获取装饰器链
chain = DecoratorUtil.get_decorator_chain(complex_operation)
print(f"装饰器链长度: {len(chain)}")
```