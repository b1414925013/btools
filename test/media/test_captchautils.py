...
class TestCaptchaUtils(unittest.TestCase):
    ...
    def test_generate_captcha_numeric(self):
        ...
        # 生成纯数字验证码
        captcha_text, image_bytes = CaptchaUtils.generate_captcha(
            captcha_type=CaptchaUtils.TYPE_NUMERIC
        )
    ...
    def test_generate_captcha_alpha(self):
        ...
        # 生成纯字母验证码
        captcha_text, image_bytes = CaptchaUtils.generate_captcha(
            captcha_type=CaptchaUtils.TYPE_ALPHA
        )
    ...
    def test_generate_captcha_alphanumeric(self):
        ...
        # 生成字母数字混合验证码
        captcha_text, image_bytes = CaptchaUtils.generate_captcha(
            captcha_type=CaptchaUtils.TYPE_ALPHANUMERIC
        )

        # 验证结果
        self.assertTrue(
            any(c.isalpha() for c in captcha_text)
            or any(c.isdigit() for c in captcha_text)
        )
        self.assertTrue(isinstance(image_bytes, bytes))
    ...
    def test_validate_captcha_case_insensitive(self):
        ...
        # 验证大小写不同但内容相同的验证码
        self.assertTrue(
            CaptchaUtils.validate_captcha(
                captcha_text.lower(), captcha_text, case_sensitive=False
            )
        )
        self.assertTrue(
            CaptchaUtils.validate_captcha(
                captcha_text.upper(), captcha_text, case_sensitive=False
            )
        )
    ...
    def test_validate_captcha_case_sensitive(self):
        ...
        if captcha_text.lower() != captcha_text:
            self.assertFalse(
                CaptchaUtils.validate_captcha(
                    captcha_text.lower(), captcha_text, case_sensitive=True
                )
            )
        if captcha_text.upper() != captcha_text:
            self.assertFalse(
                CaptchaUtils.validate_captcha(
                    captcha_text.upper(), captcha_text, case_sensitive=True
                )
            )
